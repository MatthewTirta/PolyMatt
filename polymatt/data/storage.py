...
# Path is relative to this file's location: polymatt/data/storage.py
# going up 3 levels (data -> polymatt -> repo root) then into data/
DB_PATH = Path(__file__).resolve().parent.parent.parent / "data" / "polymatt.db"
...
def save_paper_trade(pt: PaperTrade):
    """Save a new paper trade entry to the database."""
    conn = get_connection()
    try:
        conn.execute(
